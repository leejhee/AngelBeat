            ...
            { 
                ...
                for (int i = 3; i < lines.Length; i++)
                {
					...
					if(values[5] == "")
					    ...
					    data.skillDamage = Convert.ToInt64(values[5]);
					
					if(values[6] == "")
					    data.skillRangeID = default;
					else
					    data.skillRangeID = Convert.ToInt64(values[6]);
					...
					if(values[9] == "")
					    ...
					    data.skillIconImage = Convert.ToString(values[9]);
					
					if(values[10] == "")
					    data.skillTimeLine = default;
					else
					    data.skillTimeLine = Convert.ToString(values[10]);
					
					if(values[11] == "")
					    data.unlockCondition = default;
					else
					    data.unlockCondition = (SystemEnum.eSkillUnlock)Enum.Parse(typeof(SystemEnum.eSkillUnlock), values[11]);
