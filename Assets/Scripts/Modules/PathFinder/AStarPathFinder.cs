...
/// <summary>
/// AStar 알고리즘 실행 시 필요한 정보. 외부에서 만들어서 전달할 것.
/// </summary>
public class AStarParameter
{
    ...
    public DiagonalPermit   Constraint;
    public AStarParameter(AStarGrid pGrid, Node pStart, Node pTarget, DiagonalPermit constraint)
    {
        BaseGrid = pGrid;
        StartNode = pStart;
        TargetNode = pTarget;
...
public static class AStarPathFinder
{
    ...
    /// <summary>
    /// Astar 알고리즘 실행 메서드. 외부에서 파라미터 생성 후, 이 메서드로 실행
    /// </summary>
    /// <param name="param">
    ...
    public static List<GridPoint> AStarPath(AStarParameter param)
    {
        ...
        DiagonalPermit constraint =     param.Constraint;
        #endregion
    ...
    {
        var FinalPoints = new List<GridPoint>();
        while (true)
        {          
